...
def upgrade() -> None:
    ...
    if 'deliverable_packages' not in existing_tables:
        op.create_table(
    ...
    if 'deliverable_package_items' not in existing_tables:
        op.create_table(
    ...
    if 'deliverable_memos' not in existing_tables:
        op.create_table(
            'deliverable_memos',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('package_id', sa.Integer(), nullable=False),
...
def downgrade() -> None:
    ...
    if 'deliverable_memos' in existing_tables:
        indexes = [idx['name'] for idx in inspector.get_indexes('deliverable_memos')]
        if 'idx_dm_package' in indexes:
            op.drop_index('idx_dm_package', table_name='deliverable_memos')
        op.drop_table('deliverable_memos')

    if 'deliverable_package_items' in existing_tables:
        indexes = [idx['name'] for idx in inspector.get_indexes('deliverable_package_items')]
        if 'idx_dpi_package' in indexes:
    ...
    if 'deliverable_packages' in existing_tables:
        indexes = [idx['name'] for idx in inspector.get_indexes('deliverable_packages')]
        if 'idx_dp_status' in indexes:
            op.drop_index('idx_dp_status', table_name='deliverable_packages')
        if 'idx_dp_org' in indexes:
