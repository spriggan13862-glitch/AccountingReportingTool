...
function promisifyServerListen(server) {
  return async (port) => {
    return new Promise((resolve, reject) => {
      ...
      __name(onListening, "onListening");
...
function buildOrVersionNumberIntToStr(int) {
  ...
  const major = Math.floor(int / import_consts.VERSION_INT_MAJOR_BASE);
  const remainder = int % import_consts.VERSION_INT_MAJOR_BASE;
  const minor = Math.floor(remainder / import_consts.VERSION_INT_MINOR_BASE);
  const build = remainder % import_consts.VERSION_INT_MINOR_BASE;
  let str = `${major}.${minor}`;
...
var ESCAPE_NULL = "";
var REGEXP_IS_ESCAPED = new RegExp(
...
function isUrlRelative(url) {
  return import_consts.RELATIVE_URL_REGEX.test(url);
}
...
var FORBIDDEN_REGEXP = new RegExp(`^(${import_consts2.ANONYMOUS_USERNAME}|${FORBIDDEN_USERNAMES_REGEXPS.join("|")})$`, "i");
function isForbiddenUsername(username) {
  return !!username.match(import_consts2.APIFY_ID_REGEX) || !!username.match(FORBIDDEN_REGEXP);
}
...
var _WebhookPayloadTemplate = class _WebhookPayloadTemplate {
  constructor(template, allowedVariables = null, context = {}) {
    ...
    __publicField(this, "allowedVariables", allowedVariables);
...
var _CodeHashManager = class _CodeHashManager {
  constructor(secret) {
    __publicField(this, "secret", secret);
  }
  /**
