...
def upgrade() -> None:
    ...
    if 'detected_issues' not in existing_tables:
        op.create_table(
            'detected_issues',
    ...
    if 'issue_detection_thresholds' not in existing_tables:
        op.create_table(
...
def downgrade() -> None:
    ...
    if 'issue_detection_thresholds' in existing_tables:
        indexes = [idx['name'] for idx in inspector.get_indexes('issue_detection_thresholds')]
        if 'idx_thresh_entity_code' in indexes:
            op.drop_index('idx_thresh_entity_code', table_name='issue_detection_thresholds')
        op.drop_table('issue_detection_thresholds')

    if 'detected_issues' in existing_tables:
        indexes = [idx['name'] for idx in inspector.get_indexes('detected_issues')]
        if 'idx_det_issue_severity' in indexes:
            op.drop_index('idx_det_issue_severity', table_name='detected_issues')
        if 'idx_det_issue_run' in indexes:
            op.drop_index('idx_det_issue_run', table_name='detected_issues')
        if 'idx_det_issue_entity_period' in indexes:
