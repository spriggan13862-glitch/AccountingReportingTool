      ...
      <div className="bg-white rounded-xl shadow-xl border border-slate-200/80 w-full max-w-lg mx-4 p-6 animate-in zoom-in-95 duration-150">
        ...
        <div className="flex justify-end gap-2.5 mt-6">
          ...
          <button
            ...
            className="px-4 py-2 text-xs font-semibold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 cursor-pointer"
          >
      ...
      {creating && (
        <div className="flex gap-2 mb-4 bg-slate-50 p-3 rounded-lg border border-slate-150">
          ...
          <button
            ...
            onClick={() => createMutation.mutate()}
            className="px-3 py-1 text-xs font-semibold bg-indigo-600 text-white rounded-lg disabled:opacity-50 cursor-pointer"
          >
            Create
          </button>
            ...
            <div className="flex items-center gap-2">
              {!v.is_default && (
                <button
                  ...
                  className="px-2.5 py-1 text-xs font-semibold text-indigo-600 hover:text-indigo-800 cursor-pointer"
...
export function TaxonomyAdminPage() {
  ...
  const clientAccountsList = useMemo(() => {
    return accounts.filter(a => {
      ...
      if (acctTypeFilter && a.account_type?.toLowerCase() !== acctTypeFilter.toLowerCase()) return false
  ...
  }, [accounts, unmappedOnlyFilter, selectedBatchId, tbBalanceMap, leftSearchQuery, acctTypeFilter])
  ...
  const [pendingMappings, setPendingMappings] = useState<Record<number, number | ''>>({})
        ...
        >
          ...
          {selectedAccountIds.size > 0 && !node.is_subtotal && (
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 rounded bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-[10px] shrink-0 shadow-xs hover:shadow-sm"
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <div className="bg-white border border-slate-200/80 rounded-xl p-4 shadow-sm flex flex-col justify-between">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs font-bold text-indigo-600">{mappingProgressPercentage}% Complete</span>
                  </div>
                ...
                <div className="p-4 border-b border-slate-200/60 bg-slate-50/50 flex flex-col gap-3">
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                      Account Taxonomy Mapping <span className="px-2 py-0.5 rounded bg-slate-200 text-slate-700 text-[10px]">{clientAccountsList.length}</span>
                    </h3>
                    <button
                      onClick={handleAutoMap}
                      className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 cursor-pointer shadow-xs hover:shadow-sm transition-all"
                    >
                  ...
                  <div className="flex gap-2 items-center flex-wrap">
                    ...
                    <select
                      value={unmappedOnlyFilter ? 'unmapped' : 'all'}
                      onChange={(e) => setUnmappedOnlyFilter(e.target.value === 'unmapped')}
                      className="h-8 border border-slate-200 rounded-lg px-2 text-xs bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/10"
                    >
                    ...
                    <div className="flex items-center gap-1 flex-wrap">
                      {['', 'Asset', 'Liability', 'Equity', 'Revenue', 'Expense'].map(type => (
                        <button
                          key={type || 'all'}
                          type="button"
                          onClick={() => setAcctTypeFilter(type)}
                          className={cn(
                ...
                <div className="overflow-x-auto overflow-y-auto max-h-[600px]">
                  <table className="w-full text-xs min-w-[900px]">
                    <thead className="bg-slate-50 border-b border-slate-200/80 sticky top-0 z-10">
                      <tr>
                        <th className="px-3 py-2.5 text-left font-bold text-slate-500 uppercase tracking-wider w-28">Account #</th>
                        <th className="px-3 py-2.5 text-left font-bold text-slate-500 uppercase tracking-wider">Account Name</th>
                        <th className="px-3 py-2.5 text-left font-bold text-slate-500 uppercase tracking-wider w-24">Type</th>
                        <th className="px-3 py-2.5 text-left font-bold text-slate-500 uppercase tracking-wider w-24">Status</th>
                        <th className="px-3 py-2.5 text-left font-bold text-slate-500 uppercase tracking-wider">Current Mapping</th>
                        <th className="px-3 py-2.5 text-left font-bold text-slate-500 uppercase tracking-wider">Suggested Mapping</th>
                        <th className="px-3 py-2.5 text-left font-bold text-slate-500 uppercase tracking-wider w-64">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {clientAccountsList.length === 0 ? (
                        <tr>
                          <td colSpan={7} className="px-4 py-12 text-center text-slate-400">
                            ...
                            <p className="font-semibold">No accounts match selected filters</p>
                          </td>
                        </tr>
                      ) : (
                      clientAccountsList.map(acct => {
                          ...
                          const typeColor = ACCOUNT_TYPE_COLORS[typeKey] ?? 'bg-slate-100 text-slate-600 border-slate-200'
                          const pendingVal = pendingMappings[acct.id]
                          const hasPendingChange = pendingVal !== undefined

                          return (
                            <tr key={acct.id} className={cn("hover:bg-slate-50/60 transition-colors", isMapped ? "" : "bg-amber-50/20")}>
                              <td className="px-3 py-2">
                                <span className="font-mono font-semibold text-slate-600">{acct.account_number}</span>
                              </td>
                              <td className="px-3 py-2">
                                <span className="font-medium text-slate-900">{acct.account_name}</span>
                              </td>
                              <td className="px-3 py-2">
                                <span className={cn("px-2 py-0.5 rounded-full border text-[10px] font-bold capitalize", typeColor)}>
                                  {acct.account_type}
                                </span>
                              </td>
                              <td className="px-3 py-2">
                                {isMapped ? (
                                  <span className="px-2 py-0.5 rounded-full border bg-emerald-50 border-emerald-200 text-emerald-700 text-[10px] font-bold">Mapped</span>
                                ) : (
                                  <span className="px-2 py-0.5 rounded-full border bg-amber-50 border-amber-200 text-amber-700 text-[10px] font-bold">Unmapped</span>
                                )}
                              </td>
                              <td className="px-3 py-2 text-slate-600">
                                {mappedLine ? mappedLine.name : <span className="text-slate-350 italic">—</span>}
                              </td>
                              <td className="px-3 py-2">
                                {suggestedLine ? (
                                  <span className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[10px] font-bold", getConfidenceColor(suggestion.evidence))}>
                              ...
                              <td className="px-3 py-2">
                                <div className="flex items-center gap-1.5">
                                  <select
                                    value={hasPendingChange ? (pendingVal === '' ? '' : String(pendingVal)) : (acct.reporting_taxonomy_line_id ? String(acct.reporting_taxonomy_line_id) : '')}
                                    onChange={(e) => {
                                      const val = e.target.value
                                      setPendingMappings(prev => ({ ...prev, [acct.id]: val === '' ? '' : Number(val) }))
                                    }}
                                    className="flex-1 border border-slate-200 rounded px-1.5 py-1 text-[11px] bg-white focus:outline-none focus:ring-1 focus:ring-indigo-400 max-w-[180px]"
                                  >
                                    <option value="">— Unmapped —</option>
                                    {lines.filter(l => !l.is_subtotal).map(l => (
                                  ...
                                  {hasPendingChange && (
                                    <button
                                      onClick={() => {
                                        const newId = pendingVal === '' ? null : Number(pendingVal)
                                        mapAccountsMutation.mutate({ accountIds: [acct.id], taxonomyLineId: newId })
                                        setPendingMappings(prev => { const n = { ...prev }; delete n[acct.id]; return n })
                                      }}
                                      disabled={mapAccountsMutation.isPending}
                                      className="px-2 py-1 rounded bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-bold disabled:opacity-50 shrink-0"
                                    >
                                      Save
                                    </button>
                                  )}
                                  {!hasPendingChange && suggestedLine && !isMapped && (
                                    <button
                                      onClick={() => mapAccountsMutation.mutate({ accountIds: [acct.id], taxonomyLineId: suggestedLine.id })}
                                      disabled={mapAccountsMutation.isPending}
                                      className="px-2 py-1 rounded bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-700 text-[10px] font-bold disabled:opacity-50 shrink-0 flex items-center gap-0.5"
      ...
      <div className={cn("space-y-4 animate-in fade-in-50 duration-200", activeTab !== 'admin' && "hidden")}>
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 cursor-pointer shadow-xs"
          >
