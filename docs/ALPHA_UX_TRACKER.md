...
| ID | Area | Issue | Reason Deferred | Target |
|---|---|---|---|---|
| ~~UX-DEF-13~~ | ~~COA / account_path Backfill~~ | ~~Existing accounts in DB have NULL `account_path` / `depth_level`. Backfill requires calling `POST /accounts/backfill-paths` per entity.~~ | **Resolved 2026-05-27** — `POST /accounts/backfill-paths` already existed. Added `backfillPaths(entityId?)` to `accountsApi`. `SettingsPanel` gains a "Data Maintenance" section with a scoped "Backfill Account Paths" button; shows updated count on success. | — |
| ~~UX-DEF-14~~ | ~~COA / fs_sign_convention Backfill~~ | ~~Existing accounts have NULL `fs_sign_convention`. Should be populated from `normal_balance` (debit → 1, credit → -1 for most types).~~ | **Resolved 2026-05-27** — Added `POST /accounts/backfill-fs-sign` endpoint (fills NULL `fs_sign_convention` from `normal_balance`). `accountsApi.backfillFsSign(entityId?)` added. `SettingsPanel` "Data Maintenance" section also includes "Backfill FS Sign" button. | — |
...
### Tier 2 Sprint 2 — COA, Documents, FS, PDF, Pivot (2026-05-27)

| ID | Priority | Area | Issue | Fix | Status |
|---|---|---|---|---|---|
| UX-DEF-08 | Small | COA / Optional Column Visibility | COA table had no way to hide low-signal columns (Detail Type, Reporting Line, Conflict, Parent). The Settings panel only offered density and inactive-account toggles. | Added `OptionalCol` type + `loadVisibleCols` (localStorage-persisted). `SettingsPanel` gains "Optional Columns" section with per-column checkboxes. Thead filters the columns array; each optional `<td>` in `AccountRow` is conditionally rendered. Empty-state `colSpan` is now `7 + visibleColumns.size`. Preference persists across sessions via `coa-visible-cols` key. | Resolved |
| UX-DEF-03 | Small | Documents Center / Go-to-Source | `navToSource` for `pdf_import` navigated to generic `/pdf-import` with no batch context; the user had to manually find their batch. | `DocumentsPage.navToSource` now navigates to `/pdf-import?batch=<source_id>`. `PDFImportPage` reads the `?batch` query param on mount via `useEffect`+`useSearchParams`, calls `pdfImportApi.get(id)`, then jumps directly to the applied view for that batch. Deep-link works for all batches including historical ones. | Resolved |
| UX-DEF-06 | Medium | FS / Account Drilldown Enrichment | The drilldown sidebar showed JE lines but no taxonomy path breadcrumb and no period-by-period balance breakdown. | Added `drilldownTaxonomyPath` (walks `parent_id` chain on loaded `taxonomyLines` to build root→leaf path). Added `drilldownPeriodBalances` (groups `drilldownLines` by `YYYY-MM`, sums debit/credit). Drawer header now shows account number + name + taxonomy breadcrumb chips. A "Period Balances" mini table appears below the JE table when `≥2` periods have activity. | Resolved |
| UX-DEF-07 | Medium | PDF Import / Applied Lines Undo Stack | Applied lines already supported inline editing (EditableCell for account name, code, taxonomy, amount), but edits had no undo. | Added `appliedLineUndoStack` state. `onUpdateLineWithUndo` captures the pre-edit field values into a reverse patch and pushes to the stack before calling the mutation. `undoAppliedLineEdit` pops the stack and re-applies the reverse patch. An "Undo (N)" button appears in the applied control bar when the stack is non-empty; cleared on new batch load. | Resolved |
| UX-DEF-02 | Medium | PDF Preview / Section Drag-and-Drop | The GripVertical icon on preview rows was decorative; dragging between sections required using the section dropdown. | Each row's GripVertical cell is now a `draggable` span tracking `draggingLineIndexRef` (via `useRef` to avoid render churn). Each section group container has `onDragOver` (prevent default + set highlight), `onDragLeave` (clear highlight on exit), and `onDrop` (call `patchPreviewLineMutation` with the hovered section). Drop zone highlights with `ring-2 ring-indigo-200`. | Resolved |
| UX-DEF-10 | Large | Adjustment Bridge / 2D Pivot | The pivot table was 1D (row_dimension × measures). No column dimension; `column_dimensions` in the saved view schema was unused. | Added `columnDimension` + `pivotMeasure` state. `SlicerPanel` gains "Column By" dropdown (excludes current row dimension) and conditional "Pivot Measure" dropdown. `pivotMatrix` useMemo builds a `{rowValues, colValues, matrix, rowTotals, colTotals, grandTotal}` structure. The pivot summary table renders as a 2D matrix when `columnDimension` is set, with row totals, col totals, and a grand total cell. Save/load of views now persists `column_dimensions` and `pivot_measure` in `slicer_config`. | Resolved |

### Tier 2 Sprint 3 — Documents, COA Sidebar, Data Maintenance (2026-05-27)
...
| ID | Priority | Area | Issue | Fix | Status |
|---|---|---|---|---|---|
| UX-DEF-05 | Medium | Documents Center / Status-Aware Navigation | Actions column showed static "Open Source Import" button regardless of batch status. No quick link to view entity accounts. | Status-aware button: emerald+Play "Review & Apply" for ready statuses, amber+RefreshCw "Continue Import" for incomplete, blue+ExternalLink "View Import" for finalized. `sourceNavLabel()` helper + `FINALIZED/INCOMPLETE/READY_STATUSES` sets. "View Accounts" button navigates to `/coa?entity=N` for rows with `entity_id`. | Resolved |
